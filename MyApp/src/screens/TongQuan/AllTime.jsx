...
const ChartView = ({ pickedTime, data }) => {
    ...
    yearList.forEach(element => {
        for (let i = 1; i <= 12; i++) { chartLabel.push(i + "/" + element); }
    });
    const dataChart = {
        labels: chartLabel,
        datasets: [
    ...
    const countTransferText = chartLabel[0] + "-" + chartLabel[chartLabel.length - 1];
    ...
    return (
        <NavigationContainer independent={true} >
            <AllTimeStack.Navigator
                ...
                <AllTimeStack.Screen name="DetailWeekNavigator" component={DetailWeekNavigator} />
            </AllTimeStack.Navigator>
        </NavigationContainer>
    );
};
